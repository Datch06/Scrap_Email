...
Usage:
    python3 update_feuille2_with_leaders_playwright.py
"""
...
REQUEST_TIMEOUT = 15
PAUSE_SECONDS = 2.0
USE_PLAYWRIGHT_FALLBACK = True  # Activer/désactiver Playwright fallback
...
def find_siret_siren(domain, opener):
    ...
    # Phase 1: Essayer avec urllib (rapide)
    ...
    # Phase 2: Fallback Playwright si activé
    if USE_PLAYWRIGHT_FALLBACK:
        print("↻ Essai avec Playwright...", end=' ', flush=True)
        for page_path in LEGAL_PAGES[:5]:  # Tester seulement les 5 premières pages
            url = f"https://{domain}{page_path}"
    ...
    # Phase 3: Si pas trouvé avec mot-clé, chercher des séquences de 14 chiffres

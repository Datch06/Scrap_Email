#!/usr/bin/env python3
"""
Script ULTIME pour scraper LinkAvista MarketLink
Combine TOUS les filtres pour maximiser les domaines
"""
...
class LinkAvistaUltimateScraper:
    ...
    def extract_urls_from_page(self, page_num, filter_type="normal"):
        """Extraire toutes les URLs d'une page selon le filtre"""
        try:
    ...
    def scrape_ultimate(self, max_pages=100):
        """Scraper ultime : Normal + Sensitive + Google News"""
        print("\n" + "="*80)
        print("🚀 SCRAPING LINKAVISTA ULTIME - TOUS LES FILTRES COMBINÉS")
        print("="*80)
        ...
        filters = [
            ...
            ("Google News", "gnews", 20),  # Google News a moins de pages
        ]

        # PHASE 1: Extraction avec tous les filtres
        ...
        for filter_name, filter_type, max_p in filters:
            ...
            for page in range(1, max_p + 1):
                print(f"📄 Page {page:3}/{max_p} ({filter_name:15})... ", end='', flush=True)
                urls = self.extract_urls_from_page(page, filter_type)
                ...
                print(f"✅ {len(urls):3} sites (+{len(new_urls):3} nouveaux) | Total: {len(all_domains):,}")
        ...
        for idx, domain in enumerate(sorted(all_domains), 1):
            ...
            print(f"[{idx:5}/{len(all_domains)}] {domain[:40]:40} ", end='', flush=True)
            ...
            site = Site(
                ...
                email_source="linkavista_ultimate_scraping",
...
if __name__ == "__main__":
    ...
    scraper = LinkAvistaUltimateScraper(EMAIL, PASSWORD)
    scraper.scrape_ultimate(max_pages=100)
